...
'''
d1=45*5 # direction normal to curve in deg
...
u0 = np.float64(exactr(X,Y,0))+0.0
v0 = np.float64(exacti(X,Y,0))+0.0
u1 = np.float64(exactr(X,Y,1))+0.0
...
program = fd.CL_Program(ctx, 'kernal_file.c', global_vars=global_vars).program
...
plt.imshow(thewave,interpolation='none')
plt.show()
